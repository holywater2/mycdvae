...
class CDVAE_Bravais_Mono(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        
        self.lattice_dof = 4 # 4 - Monoclinic
        ...
        self.fc_num_atoms = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                      self.hparams.fc_num_layers, self.hparams.max_atoms+1)
        self.fc_lattice = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                    self.hparams.fc_num_layers, self.lattice_dof)
        self.fc_composition = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                        self.hparams.fc_num_layers, MAX_ATOMIC_NUM)
        # for property prediction.
        if self.hparams.predict_property:
            self.fc_property = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim,
                                         self.hparams.fc_num_layersß, 1)
    ...
    # FixMe: Assume that data is Monoclinic
    def predict_lattice(self, z, num_atoms):
        self.lattice_scaler.match_device(z)
        pred_lengths_and_angles = self.fc_lattice(z)  # (N, 4)
        bat_len = len(pred_lengths_and_angles)
        pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:3],
                                            torch.ones(bat_len)*90,
                                            pred_lengths_and_angles[:,3:],
                                            torch.ones(bat_len)*90,
                                            ])
        scaled_preds = self.lattice_scaler.inverse_transform(
            pred_lengths_and_angles)
...
class CDVAE_CF(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        # }
        lattice_dof_list = [0,6,4,3,2,2,1]            
        self.lattice_dof = lattice_dof_list[self.hparams.crystal_family]
    ...
    def predict_lattice(self, z, num_atoms):
        ...
        pred_lengths_and_angles = self.fc_lattice(z)  # (N, 4)
        bat_len = len(pred_lengths_and_angles)
        # print(pred_lengths_and_angles)
        # print(pred_lengths_and_angles.size())
        # print(torch.ones((bat_len,1)).size())
        # if self.hparm.crystal_family == 1:
        if self.hparams.crystal_family == 2:
            pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:3],
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                pred_lengths_and_angles[:,3:],
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                ],dim=1)
            # print("[I] changed")
            # print(pred_lengths_and_angles)
            # print(pred_lengths_and_angles.size())
        elif self.hparams.crystal_family == 3:
            pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:3],
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                ],dim=1)
        elif self.hparams.crystal_family == 4:
            pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,1:2],
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                ],dim=1)
        elif self.hparams.crystal_family == 5:
            pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,1:2],
        ...
        elif self.hparams.crystal_family == 6:
            pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,1:2],
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                torch.ones((bat_len,1)).to("cuda")*90,
                                                torch.ones((bat_len,1)).to("cuda")*120,
                                                ],dim=1)
        elif self.hparams.crystal_family == 7:
            pred_lengths_and_angles = torch.cat([pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,:1],
                                                pred_lengths_and_angles[:,:1],
                                                torch.ones((bat_len,1)).to("cuda")*90,
    ...
    def composition_loss(self, pred_composition_per_atom, target_atom_types, batch):
        target_atom_types = target_atom_types - 1
        # print(pred_composition_per_atom,target_atom_types)
        loss = F.cross_entropy(pred_composition_per_atom,
                               target_atom_types, reduction='none')
        # print(loss, loss.size())
        # print(batch.batch, batch.batch.size())
        return scatter(loss, batch.batch, reduce='mean').mean()
